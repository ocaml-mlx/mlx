(* Local copy of the compiler's Format_doc (vendored from the ocaml
   submodule) so that vendored 5.5 sources compile against host
   compilers that predate it. *)
module Format_doc = Format_doc

module Misc = struct
  include Misc

  (* Vendored from ocaml/utils/misc.ml (5.5.0) for host compilers that
     predate it. *)
  ...
  (* Plain-text rendering, matching how the upstream Misc.Style renders
     when terminal styling is disabled. *)
  module Style = struct
    let inline_code ppf s = Format_doc.fprintf ppf "%s" s
    let as_inline_code printer ppf x = printer ppf x
  end
end
...
module Clflags = struct
  ...
  (* Keyword editions (vendored from ocaml/utils/clflags.ml, 5.5.0) for
     host compilers that predate them. *)
...
(* The parser only uses these for warning-53 bookkeeping, which is
   irrelevant to a standalone preprocessor: attributes are re-checked by
   the compiler that consumes our output. *)
module Builtin_attributes = struct
  type current_phase = Parser | Invariant_check

  let register_attr _phase _name = ()
  let mark_payload_attrs_used _payload = ()
end
...
module Pprintast = struct
  ...
  module Doc = struct
    (* Upstream Pprintast.Doc.tyvar minus the keyword-escape case, which
       would need the lexer's keyword table. *)
    let tyvar ppf s =
      if String.length s >= 2 && s.[1] = '\'' then
        Format_doc.fprintf ppf "' %s" s
      else if String.equal s "_" then Format_doc.fprintf ppf "%s" s
      else Format_doc.fprintf ppf "'%s" s
  end
end
...
module Location = struct
  ...
  let errorf ?loc ?sub ?footnote:_ fmt =
    Format_doc.kasprintf (error ?loc ?sub) fmt

  let msg ?(loc = none) fmt =
    Format_doc.kasprintf (fun msg -> loc, msg) fmt
...
(* Menhir's --infer bakes canonical type paths into the generated
   parser. On a 5.5 host, astlib's Longident is an alias for the
   compiler's, so the generated code spells it Ocaml_common.Longident.t
   — a different type on any other host compiler. Shadow Ocaml_common so
   that spelling resolves to the astlib type everywhere. *)
module Ocaml_common = struct
  (* Capture the astlib Longident before the include brings the real
     Ocaml_common.Longident into scope. *)
  module Portable_longident = Longident
  include Ocaml_common
  module Longident = Portable_longident
end
