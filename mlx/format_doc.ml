...
module Doc = struct
  ...
  let rec split_on_close opened rbefore = function
   ...
   (* mlx: local replacement for Format.utf_8_scalar_width (stdlib >=
      5.4) so this file compiles on older host compilers: count the
      Unicode scalars in a UTF-8 string. *)
   let utf_8_scalar_width s =
   ...
   let rec approx_len acc = function
     | [] -> Some acc
     | Text x :: r->
         let len = utf_8_scalar_width x in
