...
let modulize s =
  match Misc.Utf8_lexeme.capitalize s with Ok x | Error x -> x
...
let lax_modname_from_source source_file = source_file |> stem |> modulize
...
let mli_from_artifact f = Artifact.prefix f ^ ".mli"
...
let find_normalized_cmi f =
  ...
  let filename = Load_path.find_normalized filename in
  {
    Artifact.filename;
    modname = modname f;
    source_file = Some f.source_file;
  }
