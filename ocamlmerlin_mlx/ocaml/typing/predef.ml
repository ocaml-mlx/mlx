...
type data_type_constr = [ `Bool | `Unit | `Exn | `Eff | `List | `Option ]
type type_constr = [ abstract_type_constr | data_type_constr ]
...
let path_of_type_constr typ = Pident (ident_of_type_constr typ)
...
let find_type_constr =
  let all_predef_paths =
    ...
    |> List.fold_left (fun m (k, v) -> Path.Map.add k v m) Path.Map.empty
  in
...
let decl_of_type_constr tconstr =
  ...
  let decl0 ?(immediate = Type_immediacy.Unknown)
  ...
  in
  let decl1 ~variance ?(separability = Separability.Ind)
  ...
  match tconstr with
  | `Int | `Char -> decl0 ~immediate:Always ()
  | `String | `Bytes | `Float | `Floatarray | `Nativeint | `Int32 | `Int64
  | `Extension_constructor ->
      decl0 ()
  | `Bool ->
      let kind = variant [ cstr ident_false []; cstr ident_true [] ] in
  ...
  | `Continuation ->
      ...
      decl2 ~variance ()
  | `Array | `Atomic_loc -> decl1 ~variance:Variance.full ()
  | `Iarray -> decl1 ~variance:Variance.covariant ()
  | `List ->
      let kind tvar =
        variant
...
let build_initial_env add_type add_extension empty_env =
  ...
  List.fold_left
    (fun env tconstr ->
      add_type
        (ident_of_type_constr tconstr)
        (decl_of_type_constr tconstr)
        env)
  ...
  |> add_extension ident_assert_failure
       [
         newgenty
           (Ttuple [ None, type_string; None, type_int; None, type_int ]);
       ]
